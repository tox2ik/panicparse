...
// TestPanic runs github.com/maruel/panicparse/cmd/panic with every
...
func identifyPanicwebSignature(t *testing.T, b *Bucket, pwebDir string) panicwebSignatureType {
	...
	if b.CreatedBy.Calls[0].ImportPath == "github.com/maruel/panicparse/cmd/panicweb/internal" && b.CreatedBy.Calls[0].Func.Name == "GetAsync" {
		// TODO(maruel): More checks.
		return pstClient
	}

	// Now find the two goroutine started by main.
	if b.CreatedBy.Calls[0].ImportPath == "github.com/maruel/panicparse/cmd/panicweb" && b.CreatedBy.Calls[0].Func.ImportPath == "main" && b.CreatedBy.Calls[0].Func.Name == "main" {
		if b.State == "IO wait" {
			return pstServe
		}
		if b.State == "chan receive" {
...
func getPanicParseDir(t *testing.T) string {
	...
	// simply our life.
	return strings.Replace(filepath.Dir(filepath.Dir(thisDir)), "\\", "/", -1)
}
